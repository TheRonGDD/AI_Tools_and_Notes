...
Model IDs and error codes below were verified against live API calls on 2026-09-20, not taken from vendor blog posts. The one exception is Groq, and it is labelled as such.
...
**This is the one thing this refresh could not settle.** August's guide recorded, from a live API call, that Groq had dropped every Llama chat model. As of 2026-09-20, Groq's own documentation lists Llama 3.1 8B and Llama 3.3 70B as current production models. Either Groq restored them or the docs are stale, and no Groq key was available during this review to find out. The [Groq section](Free_AI_API_Keys.md#groq) is flagged accordingly and still shows August's live data. Send one throwaway request before trusting a Llama model ID there, in either direction.
...
- **[Groq](https://groq.com)** — Fastest free inference API (LPU hardware), no credit card. Catalog size is currently unclear: see the [open question](Free_AI_API_Keys.md#groq).
