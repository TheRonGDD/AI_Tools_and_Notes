...
- **Qwen 3.8 27B: get the model ID right, because both free hosts spell it differently.** This is the model the most people are currently trying to run, and there are three separate traps. On **Groq's free API** it is `qwen/qwen3.8-27b`, and the previous `qwen/qwen3.6-27b` ID — which this repo listed in August — **is gone from the catalog**, so any guide written before September hands you a dead ID. Running it **locally under Ollama** it is `qwen3.8:27b`, a colon rather than a hyphen, with a `qwen3.8:27b-mlx` build for Apple Silicon. It is also on **OpenRouter** as `qwen/qwen3.8-27b:free`. And the **context window differs by host**: Ollama's library lists 256K, OpenRouter serves the full 262,144, and Groq serves it at 131,042 — so if you need the long context for free, OpenRouter is the host to use, not Groq. All three verified live 2026-09-20.
...
- **OpenRouter's catalog can be verified with no key at all**, which makes it the cheapest fact-check in this repo: `GET https://openrouter.ai/api/v1/models` is public and returns pricing and context length for all 446 models. Anything with `pricing.prompt` and `pricing.completion` both `"0"` is free right now — 24 models as of 2026-09-20, now listed in full in the [OpenRouter section](Free_AI_API_Keys.md#openrouter). Two stand out: `qwen/qwen3.8-27b:free` at the full 262,144 context, and `z-ai/glm-5.2:free`, still running at 32,768 despite having been retired on NVIDIA NIM on 2026-08-21.
...
- **Union Alpha**, the stealth model OpenCode promoted on 2026-09-16 as free for a week. It was worth a look: agentic coding focus, image input, 262K context, no training on your data. **It is already gone.** As of 2026-09-20 `stealth/union-alpha` returns `404` from OpenRouter's API; the model was revealed as **Pareto** by Unbiased and now sits at `unbiased/pareto` charging $2.50/M input and $7.50/M output. The free window ran days, not the advertised week. Stealth-model promotions are vendors buying evaluation data, not free tiers, and they fail this repo's ongoing-free bar by design — but the speed of this one is the useful part: if you had wired it into a config on announcement day, it would have broken before you finished the week.
